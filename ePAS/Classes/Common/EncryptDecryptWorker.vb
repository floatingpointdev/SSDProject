...
Public Class EncryptDecryptWorker
    '=====================================TripleDES CBC Mode=============================================
    ...
    Public Function MD5GenerateHash(ByVal SourceText As String) As String
    ...
    Public Function GetHexStringFromBytes(ByVal bytes() As Byte, Optional ByVal len As Integer = -1, Optional ByVal spaces As Boolean = False) As String
    ...
    Public Function GetCharStringFromBytes(ByVal bytes() As Byte) As String
        Dim sTemp As String = ""

        For i As Integer = 0 To bytes.Length - 1
            sTemp = sTemp + Chr(bytes(i))
        Next
        Return sTemp
    End Function

    Public Function DiversifyKey(ByVal sKey As String, ByVal sInput As String) As String
        Dim des3Prov As New TripleDESCryptoServiceProvider()

        'Dim sKey As String = ""
        Dim sKeyHex As String = ""
        Dim bKey As Byte()
        ...
        Dim iv(7) As Byte 'blank

        Dim sDivKey As String = ""

        'validations
        If sKey.Trim().Length = 0 Or sKey.Trim().Length > 16 Then
            Return sDivKey
        End If

        sKey = sKey.Trim().PadRight(16, "0")

        'validations-end

        sKeyHex = oClsCtrl.GetHexStrOfStr(sKey.PadRight(16, "0")) 'the key should be 16 bytes
        bKey = HexStringToBytes(sKeyHex)
        sInput = oClsCtrl.GetHexStrOfStr(sInput.Substring(0, 8)) 'the input/message, should be in multiple of 8
        bInput = HexStringToBytes(sInput)
        ReDim bOutput(bInput.Length - 1) 'output length should be the same as the input
        ...
        Dim netDes As ICryptoTransform = des3Prov.CreateEncryptor(bKey, iv)
        netDes.TransformBlock(bInput, 0, bInput.Length, bOutput, 0)
        sDivKey = GetHexStringFromBytes(bOutput).Substring(0, 12)
